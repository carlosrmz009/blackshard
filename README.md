...
- **ClamAV compatibility layer:** the development build pins the official ClamAV 1.5.2 x64 runtime by SHA-256. Official `freshclam` performs scheduled database retrieval; `sigtool` verifies each CVD/CLD before versioned activation and unpacking. Blackshard loads SHA-256 `.hsb`/`.hsu` records into its native exact-match index and streams exact duplicated-handle bytes over `INSTREAM` to one resident, job-bounded `clamd` child on a random loopback-only port. ClamAV detections can block but remain ineligible for automatic quarantine until clean-corpus false-positive qualification is complete.
...
Blackshard consumes the Windows AMSI API and the development/production packaging includes separate x64 and x86 Blackshard AMSI provider DLLs. The provider uses a dedicated, schema-restricted local endpoint and cannot issue management commands. Structured candidates also pass through an independently sandboxed parser worker. Readiness remains degraded until the FreshClam generation, ClamAV worker, parser worker, driver protocol, storage, IPC, and end-to-end enforcement probe are all healthy. This remains pre-release functionality until clean-VM registration, host-compatibility, latency, bypass, corpus, and independent-review gates pass. Blackshard does not yet self-update application/driver binaries in the background or implement Shardnet/P2P.
...
- Internet access on the first complete package build to download the pinned official ClamAV 1.5.2 x64 runtime (about 218 MiB); subsequent builds reuse the hash-verified local cache
...
`deploy.ps1` also runs `tools\Get-ClamAvRuntime.ps1`, verifies the upstream ClamAV archive against the repository-pinned SHA-256, and creates `dist\clamav-runtime.zip`. The development installer expands that runtime under `%ProgramFiles%\Blackshard\ClamAV`; it is required for a healthy readiness state.

The ClamAV runtime remains a separate GPL-2.0 program and is not linked into Blackshard. Blackshard starts it as a sidecar and communicates through ClamAV's documented streaming protocol. The current bundled-runtime path is for private test builds; public binary redistribution still requires a dedicated license/notice/source-offer review.
...
Clean CI builds also emit `BlackshardVmSetup.exe`, a one-click **development VM installer** with a Blackshard-styled graphical progress/error interface. It bundles the agent, pinned ClamAV runtime, and test minifilter; configures test-signing; resumes automatically after reboot; installs and verifies the components; repairs partial attempts; and registers uninstall. It refuses systems that Windows does not identify as virtual machines and does not weaken the production installer's signature gates. See [the testing guide](docs/TESTING.md#one-click-vm-setup).
